...
creates tables/columns from the current model state. Guards prevent
errors when objects already exist.
"""
...
def upgrade() -> None:
    ...
    if not insp.has_table("extracted_otp_codes"):
        op.create_table(
            "extracted_otp_codes",
    ...
    columns = [c["name"] for c in insp.get_columns("user_settings")]
    if "approval_mode_otp" not in columns:
        op.add_column(
            "user_settings",
            sa.Column(
                "approval_mode_otp",
